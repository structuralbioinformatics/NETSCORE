#include "ppin.h"
float  ModifyNode(iter,dnode,dedge,size,edges,indirect,linker,scr,d_scr,n_scr,escr,distribution,ndstr,rnd,maxmin)
int       iter,size,edges,linker,indirect,*rnd;
...
edge     *dedge;
{
  ...
  diff=0.0;
  if (iter==0){
     error=InitZscorePopulation(d_scr,n_scr,dnode,dedge,size,edges,linker,distribution,ndstr,rnd,maxmin,scr);
  }else{
    ...
    diff=CmpMatrix(d_old,d_scr,n_scr_old,size);
    error=diff;
    ZscorePopulation(d_scr,n_scr,dnode,dedge,size,edges,linker,distribution,ndstr,rnd,maxmin,scr);
