...
/*
Find the median of X(1), ... , X(N), using as much of the quickCsort
...
{ int i, j;
  ...
  { int loop;
    ...
    /* The basic quickCsort algorithm to move all values <= the Csort key (XMED)
...
static const float* Csortdata = NULL; /* used in the quickCsort algorithm */

/* ---------------------------------------------------------------------- */

static
int compare(const void* a, const void* b)
/* Helper function for Csort. Previously, this was a nested function under
 * Csort, which is not allowed under ANSI C.
 */
{ const int i1 = *(const int*)a;
  const int i2 = *(const int*)b;
  const float term1 = Csortdata[i1];
  const float term2 = Csortdata[i2];
...
void Csort(int n, const float data[], int index[])
...
static void
freedatamask(int n, float** data, int** mask)
{ int i;
  for (i = 0; i < n; i++)
  { free(mask[i]);
    free(data[i]);
...
static int svd(int m, int n, float** u, float w[], float** vt)
/*
...
{ int i, j, k, i1, k1, l1, its;
  ...
  float scale = 0.0;
  float anorm = 0.0;
  float* rv1 = malloc(n*sizeof(float));
  if (!rv1) return -1;
  if (m >= n)
  { /* Householder reduction to bidiagonal form */
    ...
    { k1 = k-1;
      ...
      { for (l = k; l >= 0; l--)
        { l1 = l-1;
          if (fabsf(rv1[l]) + anorm == anorm) break;
          /* rv1[0] is always zero, so there is no exit
           * through the bottom of the loop */
          if (fabsf(w[l1]) + anorm == anorm)
  ...
  { /* Householder reduction to bidiagonal form */
    ...
    { l = i + 1;
      ...
      s = 0.0;
      scale = 0.0;
      for (k = i; k < n; k++) scale += fabsf(u[i][k]);
      if (scale != 0.0)
      { for (k = i; k < n; k++)
        { u[i][k] /= scale;
          s += u[i][k]*u[i][k];
      ...
      scale = 0.0;
      if (i<m-1)
      { for (k = l; k < m; k++) scale += fabsf(u[k][i]);
        if (scale != 0.0)
        { for (k = l; k < m; k++)
          { u[k][i] /= scale;
            s += u[k][i] * u[k][i];
...
The arrays u, v, and w are Csorted according to eigenvalue, with the largest
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { float term = data1[index1][i] - data2[index2][i];
        ...
        tweight += weight[i];
        if (maxi<= term*term)maxi=term*term;
      }
    }
  }
  else
...
{ float result = 0.;
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { float term = data1[index1][i] - data2[index2][i];
        result = result + weight[i]*fabsf(term);
        tweight += weight[i];
        if (maxi<= fabsf(term))maxi=fabsf(term);
      }
    }
  }
  else
...
static
float mutualinformation (int n, float** data1, float** data2, int** mask1,
  int** mask2, const float weight[], int index1, int index2, int transpose)
{
 /* The method Ignores weight */
  int   i,j,k,kmax,n1,m1,ni,nj;
  int   kbins1,kbins2,SelectNbins();
  int   **nn,**imatrix();
  float max1,max2,min1,min2,h1,h2,h,hx,hy,hygx,hxgy,uygx,uxgy,uxy,result;
  float stepi1,stepi2,stepj1,stepj2;
  float *term1,*term2;
  float *fvector();
  void  free_fvector(),free_imatrix(),cntab2();
  term1=fvector(0,n+1);
  term2=fvector(0,n+1);
  //printf("Correlation by MI\n");
  if (transpose==0) /* Calculate the distance between two rows */
  { 
    j=1;
    for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      {  term1[j] = data1[index1][i];
         term2[j] = data2[index2][i];
         j++;
      }
    }
  }
  ...
  { 
    ...
    for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      {  term1[j] = data1[i][index1];
         term2[j] = data2[i][index2];
         j++;
      }
    }
  }
  ...
  //printf("Size N1=%d M1=%d N=%d\n",n1,m1,n);
  kbins1= SelectNbins(term1,n1,m1,kmax); 
  //printf("Kbins1=%d\n",kbins1);
  kbins2= SelectNbins(term2,n1,m1,kmax); 
  //printf("Kbins2=%d\n",kbins2);
  ...
  for (i=n1;i<m1;i++){
    if (i==n1 || term1[i]>max1) max1=term1[i];
    if (i==n1 || term1[i]<min1) min1=term1[i];
    }
  for (i=n1;i<m1;i++){
    if (i==n1 || term2[i]>max2) max2=term2[i];
    if (i==n1 || term2[i]<min2) min2=term2[i];
    }
  h1=(max1-min1)/kbins1;
  h2=(max2-min2)/kbins2;
  //printf("H1 %f H2 %f Kbins1 %d Kbins2 %d\n",h1,h2,kbins1,kbins2);
  ...
  for (j=-1;j<kbins2+1;j++){
    ...
    for (k=n1;k<m1;k++)
    {
       if (   term1[k]>=stepi1 && term1[k]<stepi2 
           && term2[k]>=stepj1 && term2[k]<stepj2) nn[i+2][j+2]++;
    }
  ...
  free_imatrix(nn,0,kbins1+3,0,kbins2+3);
  free_fvector(term1,0,n+1);
  free_fvector(term2,0,n+1);
...
static
float cramerV (int n, float** data1, float** data2, int** mask1,
  int** mask2, const float weight[], int index1, int index2, int transpose)
{
 /* The method Ignores weight */
  int   i,j,k,kmax,n1,m1,ni,nj;
  int   kbins1,kbins2,SelectNbins();
  int   **nn,**imatrix();
  float max1,max2,min1,min2,h1,h2,chisq,df,prob,cramrv,ccc,result;
  float stepi1,stepi2,stepj1,stepj2;
  float *term1,*term2;
  float *fvector();
  void  free_fvector(),free_imatrix(),cntab2();
...
static
float contingencyC (int n, float** data1, float** data2, int** mask1,
  int** mask2, const float weight[], int index1, int index2, int transpose)
{
 /* The method Ignores weight */
  int   i,j,k,kmax,n1,m1,ni,nj;
...
{ float result = 0.;
  ...
  //printf("Correlation WEIGHT= %f SUM1= %f SUM2= %f DENOM1= %f DENOM2= %f RSULT= %f \n",tweight,sum1,sum2,denom1,denom2,result);
...
{ float result = 0.;
  ...
  if (denom1 <= 0) return 1; /* include '<' to deal with roundoff errors */
  if (denom2 <= 0) return 1; /* include '<' to deal with roundoff errors */
  result = fabsf(result) / sqrt(denom1*denom2);
  //result = 1. - result;
  return result;
}
...
{ //printf("Correlation selected DIST=%c\n",dist);
  switch(dist)
  { case 'e': return &euclid;
    ...
    case 'm': return &mutualinformation;
    case 'v': return &cramerV;
    case 'g': return &contingencyC;
...
static float(*fsetmetric(int *meth)) 
  (int, float**, float**, int**, int**, const float[], int, int, int)
{ 
  switch(meth[5])
  { case 7: return &euclid;
    case 8: return &cityblock;
    case 9: return &mutualinformation;
    case 10: return &cramerV;
    case 11: return &contingencyC;
    case 2: return &correlation;
    case 4: return &acorrelation;
    case 1: return &ucorrelation;
    case 3: return &uacorrelation;
    case 5: return &spearman;
    case 6: return &kendall;
    default: return &correlation;
  }
  return NULL; /* Never get here */
}
...
dist=='m': Mutual Information UXY
dist=='g': Contingency coefficient C
dist=='v': Cramers' V
...
static
tNode* pclcluster (int nrows, int ncolumns, float** data, int** mask,
...
{ int i, j;
  ...
  result = malloc(nnodes*sizeof(tNode));
  if(!result)
  { free(distid);
    return NULL;
  }
...
{ const tNode* node1 = (const tNode*)a;
  const tNode* node2 = (const tNode*)b;
...
static
tNode* pslcluster (int nrows, int ncolumns, float** data, int** mask,
  float weight[], float** distmatrix, char dist, int transpose)
...
{ int i, j, k;
  ...
  result = malloc(nelements*sizeof(tNode));
  if(!result)
  { free(vector);
    free(index);
    free(temp);
  ...
  qsort(result, nnodes, sizeof(tNode), nodecompare);
  ...
  result = realloc(result, nnodes*sizeof(tNode));

  return result;
}
/* ******************************************************************** */

static tNode* pmlcluster (int nelements, float** distmatrix)
/*
...
{ int j;
  ...
  result = malloc((nelements-1)*sizeof(tNode));
  if (!result)
  { free(clusterid);
    return NULL;
  }
...
static tNode* palcluster (int nelements, float** distmatrix)
/*
Purpose
=======
...
float **XSimilarityMatrix(protein,xprobe,size,number_phe,number_samples,xmethod,clusparam,shortest_path,dist)
node      *protein;
express   *xprobe;
int        size,number_phe,number_samples[],*xmethod,**shortest_path;
float     *clusparam;
char       dist;
{
 FILE   *OUT;
 float **c,*a,*b,sa,sb,center1,center2;
 float  *w,**data1,**data2;
 float   correlat;
 int     i,j,ii,jj,k,n,m,m1,m2,dim,**mask1,**mask2;
 int     status,syscall;
 pid_t   pid;
 float   PearsonScore();
 float  *fvector();
 float **matrix();
 int   **imatrix();
 void    free_matrix(),free_fvector(),free_imatrix(),nrerror(),NormalizeCLR(),NormalizeAPC(),NormalizeASC();
 /* Set the metric function as indicated by dist */
 float (*metric) 
    (int, float**, float**, int**, int**, const float[], int, int, int) = setmetric(dist);


 c=matrix(0,size,0,size);
 dim=0;
 printf("Phenotypes %d\n",number_phe);
 for (i=0;i<number_phe;i++) printf("Samples[%d] %d\n",i,number_samples[i]);
 for (i=0;i<number_phe;i++)dim+=number_samples[i];
 a=fvector(0,dim);
 b=fvector(0,dim);
 w=fvector(0,dim);
 data1=matrix(0,1,0,dim);
 data2=matrix(0,1,0,dim);
 mask1=imatrix(0,1,0,dim);
 mask2=imatrix(0,1,0,dim);

 //printf("Open  .network_of_similarity.dat\n");
 OUT=fopen(".network_of_similarity.dat","r");

 if (OUT) 
 {
  fclose(OUT);
  printf("Execute: mv .network_of_similarity.dat .network_of_similarity.dat_old\n");
  fflush(stdout);
  pid=fork();
  if (pid<0) {fprintf(stderr,"Can't fork, error(%d) %s \n",errno,strerror(errno));nrerror("FORK() is not possible\n");}
  if (pid==0){ syscall=system("\\mv .network_of_similarity.dat .network_of_similarity.dat_old"); exit(syscall);}
  else       { wait(&status); printf("Status %d\n",WEXITSTATUS(status));}
  //system("\\mv .network_of_similarity.dat .network_of_similarity.dat_old");
  OUT=fopen(".network_of_similarity.dat","w");
 }else{
 //printf("close  .network_of_similarity.dat\n");
  //fclose(OUT);
 //printf("Open  .network_of_similarity.dat\n");
  OUT=fopen(".network_of_similarity.dat","w");
 }  
      
 if (xmethod[7]==1) printf("Matrix of Similarity\n");
 for (m=0;m<size;m++)for (n=0;n<size;n++){c[m][n]=0.0;}
 for (m=0;m<size;m++){
 for (n=m+1;n<size;n++){
  //printf("Test %d %d Split %d %d Path %d\n",n,m,xprobe[m].split,xprobe[n].split,shortest_path[n][m]);
 // printf("Xmethod[6]%d parameter[5]=%f \n",xmethod[6],clusparam[5]);
  if (xmethod[6]==0 || xmethod[6]==1 || (xmethod[6]==2 && shortest_path[n][m]<= clusparam[5]) ){
  //printf("Split[m]=%d Split[n]=%d #Phe %d \n",xprobe[m].split,xprobe[n].split,number_phe);
  for (ii=0;ii<xprobe[m].split;ii++){
  for (jj=0;jj<xprobe[n].split;jj++){
    for (i=0;i<dim;i++){ 
         data1[0][i]=0.0;
         data2[0][i]=0.0;
         mask1[0][i]=0;
    ...
    for (j=0;j<number_samples[i];j++){
         if (xmethod[8]==1){
          if (xprobe[n].fragment[jj].phenotype[i].defined[j]>0 && xprobe[n].fragment[jj].phenotype[i].sample[j]<=0) xprobe[n].fragment[jj].phenotype[i].defined[j]=0;
          if (xprobe[m].fragment[ii].phenotype[i].defined[j]>0 && xprobe[m].fragment[ii].phenotype[i].sample[j]<=0) xprobe[m].fragment[ii].phenotype[i].defined[j]=0;
         }
         mask1[0][k]=xprobe[n].fragment[jj].phenotype[i].defined[j];
         mask2[0][k]=xprobe[m].fragment[ii].phenotype[i].defined[j];
        // printf("Mask1[%d]= %d  N %d JJ %d I %d J %d \n",k,mask1[0][k],n,jj,i,j);
        // printf("\tNode[%d]Probe[%d][%d][%d]: \t%s  \tExpression: %10.3e\tMask: %5d\n",n,jj,i,j,xprobe[n].fragment[jj].name,xprobe[n].fragment[jj].phenotype[i].sample[j],xprobe[n].fragment[jj].phenotype[i].defined[j]);
        // printf("Mask2[%d]= %d  M %d II %d I %d J %d\n",k,mask2[0][k],m,ii,i,j);
        // printf("\tNode[%d]Probe[%d][%d][%d]: \t%s  \tExpression: %10.3e\tMask: %5d\n",m,ii,i,j,xprobe[m].fragment[ii].name,xprobe[m].fragment[ii].phenotype[i].sample[j],xprobe[m].fragment[ii].phenotype[i].defined[j]);
          if (xmethod[8]==1){
            if (mask1[0][k]==1) data1[0][k]=logf(xprobe[n].fragment[jj].phenotype[i].sample[j]);
            if (mask2[0][k]==1) data2[0][k]=logf(xprobe[m].fragment[ii].phenotype[i].sample[j]);
            }else{
            data1[0][k]=xprobe[n].fragment[jj].phenotype[i].sample[j];
            data2[0][k]=xprobe[m].fragment[ii].phenotype[i].sample[j];
            }
         if (xprobe[n].fragment[jj].phenotype[i].defined[j]>0 ){
          a[m1]=data1[0][k];
          sa+=a[m1]*a[m1];
          m1++;
          }
         if (xprobe[m].fragment[ii].phenotype[i].defined[j]>0){
          b[m2]=data2[0][k];
          sb+=b[m2]*b[m2];
          m2++;
          }
...
/*
   printf("\n");
   printf("Vector Xpress (L) A[%d]:\t",n);
   for (i=0;i<dim;i++)if (mask1[0][i]==1)printf("%e\t",data1[0][i]);
   printf("\n");/B
   printf("Vector Xpress (L) B[%d]:\t",m);
   for (i=0;i<dim;i++)if (mask1[0][i]==1)printf("%e\t",data2[0][i]);
   printf("\n");
*/
     if (m1>0) sa=sa/m1;
     if (m2>0) sb=sb/m2;
     ...
     case 1:
           if (m1>0){
             center1=mean(m1,a);
             for (i=0;i<dim;i++) if (mask1[0][i]==1) data1[0][i]=data1[0][i]-center1;
             }
           if (m2>0){
             center2=mean(m2,b);
             for (i=0;i<dim;i++) if (mask2[0][i]==1) data2[0][i]=data2[0][i]-center2;
             }
           break;
     case 2:
           if (m1>0){
...
/*
   printf("Vector Xpress (C) A[%d]:\t",n);
   for (i=0;i<dim;i++)if (mask1[0][i]==1)printf("%e\t",data1[0][i]);
   printf("\n");
   printf("Vector Xpress (C) B[%d]:\t",m);
   for (i=0;i<dim;i++)if (mask1[0][i]==1)printf("%e\t",data2[0][i]);
   printf("\n");
*/
     if (xmethod[9]==1){
        center1=mean(m1,a);
        center2=mean(m2,b);
        sa=sa-center1*center1;
        sb=sb-center2*center2;
        if (sa>0)for (i=0;i<dim;i++) if (mask1[0][i]==1) data1[0][i]=data1[0][i]/sa;
        if (sb>0)for (i=0;i<dim;i++) if (mask1[0][i]==1) data2[0][i]=data2[0][i]/sb;
        }
/*
   printf("Vector Xpress A[%d]:\t",n);
   for (i=0;i<dim;i++)if (mask1[0][i]==1) printf("%e\t",data1[0][i]);
   printf("\n");
   printf("Vector Xpress B[%d]:\t",m);
   for (i=0;i<dim;i++)if (mask1[0][i]==1) printf("%e\t",data2[0][i]);
   printf("\n");
*/
    switch(xmethod[3]){
    case 0:
         /*correlation=PearsonScore(a,b,k);*/
         correlat=metric(dim,data1, data2, mask1, mask2, w, 0, 0, 0);
         break;
    case 1:
         /*correlation=PearsonScore(a,b,k);*/
         correlat=metric(dim,data1, data2, mask1, mask2, w, 0, 0, 0);
    ...
    case 5:
         ...
         break;
    case 6:
         /*correlation=-1*PearsonScore(a,b,k);*/
         correlat=-1*metric(dim,data1, data2, mask1, mask2, w, 0, 0, 0);
         break;
    default:
         /*correlation=PearsonScore(a,b,k);*/
         correlat=metric(dim,data1, data2, mask1, mask2, w, 0, 0, 0);
    ...
    if (xmethod[2]==3)correlat=fabsf(correlat);

/*
    if (correlat>0){ printf("Large correlation Correl[%d][%d]= %f SPLIT=%d and %d \n",n,m,correlat,xprobe[n].split,xprobe[m].split);
                        printf("Shortest Path[%d][%d] = %d Param= %f Log(Param)= %f Factor %f \n",n,m,shortest_path[n][m],clusparam[5],logf(clusparam[5]),
                              expf((1-shortest_path[n][m])*logf(clusparam[5])));
                      }
    printf("C[%d][%d]=%f\n",m,n,correlat);
*/
    ...
    switch(xmethod[4]){ 
    case 0:
         c[m][n]+=correlat/(xprobe[n].split*xprobe[m].split);
         c[n][m]+=correlat/(xprobe[n].split*xprobe[m].split);
         break;
    case 1:
         if (ii==0 && jj==0)c[m][n]=c[n][m]=correlat;
         if (fabsf(c[m][n])>fabsf(correlat))c[m][n]=correlat;
         if (fabsf(c[n][m])>fabsf(correlat))c[n][m]=correlat;
         break;
    case 2:
         if (ii==0 && jj==0)c[m][n]=c[n][m]=correlat;
         if (fabsf(c[m][n])<fabsf(correlat))c[m][n]=correlat;
    ...
    //printf("Modify C[%d][%d]=%f and %f  %f\n",m,n,correlat, c[m][n],c[n][m]);
    if (xmethod[7]==1 && c[m][n]>0) {printf("Correlation before normalization [ %d ][ %d ] = %f / %f = %e and %e \n",m, n, correlat, xprobe[n].split*xprobe[m].split, c[m][n], c[n][m]);}

    }}}}

   }} 

 //if (xmethod[12]==1) NormalizeCLR(c,size,size); 
 switch(xmethod[12]){
 case 0:
      break;
 case 1:
      NormalizeCLR(c,size,size);
      break;
 case 2:
      NormalizeAPC(c,size);
      break;
 ...
 for (m=0;m<size;m++){
  if (xmethod[16]==1){
    c[m][m]=1.0;
    if (xmethod[7]==1) printf("Accepted: %s %s C[%d][%d]=%f\n",protein[m].name1,protein[m].name1,m,m,c[m][m]);
    if (xmethod[11]==1 || xmethod[7]==1) fprintf(OUT,"%s %s %f\n",protein[m].name1,protein[m].name1,c[m][m]);
    }
 for (n=m+1;n<size;n++){   
    if (xmethod[7]==1 && c[m][n]>0) printf("SIZE: %d COMPARE %s %s C[%d][%d]=%f B=LOG(C)=%f A=(1-s_path)*B:%f Exp(A): %f \n",size,protein[m].name1,protein[n].name1,m,n,c[m][n],logf(clusparam[5]),(1-shortest_path[n][m])*logf(clusparam[5]),expf((1-shortest_path[n][m])*logf(clusparam[5])) );
    if (xmethod[6]==1) c[m][n]=c[m][n]*expf((1-shortest_path[n][m])*logf(clusparam[5]));
    if (clusparam[12]==0 || c[m][n] > clusparam[12]){
     if (xmethod[7]==1) printf("Accepted: %s %s %f CLUSPARAM[12]= %f\n",protein[m].name1,protein[n].name1,c[m][n],clusparam[12]);
     if (xmethod[11]==1 || xmethod[7]==1) fprintf(OUT,"%s %s %f\n",protein[m].name1,protein[n].name1,c[m][n]);
     }
   }}

 fflush(OUT);
 fclose(OUT);
 free_fvector(w,0,dim);
 free_fvector(a,0,dim);
 free_fvector(b,0,dim);
 free_matrix(data1,0,1,0,dim);
 free_matrix(data2,0,1,0,dim);
 free_imatrix(mask1,0,1,0,dim);
 free_imatrix(mask2,0,1,0,dim);
 return c;
}
