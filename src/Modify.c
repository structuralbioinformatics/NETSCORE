#include "ppin.h"
float *Modify(iter,diter,interaction,protein,dnode,dedge,edges,size,dedges,dsize,linker,edgescr,nodescr,dnodescr,d_edgescr,n_edgescr,d_nodescr,n_nodescr,distribution,ndstr,edistribution,nedstr,rnd,maxmin)
...
edge     *interaction,*dedge;
{
 ...
 max2[3]=minde;
 max2[4]=MAXID;
 error[0]=ModifyNode(iter,protein,interaction,size,edges,indirect,linker_node,nodescr,d_nodescr,n_nodescr,edgescr,distribution,ndstr,rnd,max2);
 ...
 // n=iter;
  ...
  max2[3]=max_dummy;
  max2[4]=MAXIDE;
  error[1]=ModifyNode(n,dnode,dedge,edges,dedges,indirect,linker_edge,edgescr,d_edgescr,n_edgescr,dnodescr,edistribution,nedstr,rnd,max2);
