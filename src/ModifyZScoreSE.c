#include "ppin.h"
float *ModifyZScoreS(iter,diter,interaction,protein,dnode,dedge,edges,size,dedges,dsize,linker,edgescr,nodescr,dnodescr,d_edgescr,n_edgescr,d_nodescr,n_nodescr,swap,swap_edges,rnd,maxmin)
int       iter,diter,edges,size,dsize,dedges,linker[],**swap,**swap_edges,*rnd;
...
edge     *interaction,*dedge;
{
 float *error;
 float *fvector();
 float  ModifyNodeZScoreS(),ModifyDual();
 ...
 max2[3]=minde;
 max2[4]=MAXID;
 error[0]=ModifyNodeZScoreS(iter,protein,interaction,size,edges,indirect,linker_node,linker_degree,nodescr,d_nodescr,n_nodescr,edgescr,swap,rnd,max2);
 ...
 // n=iter;
  ...
  printf("\nScore-Flow on Dual Graph (edges=%d)\n",edges);
  max2[0]=maxe;
  max2[1]=mine;
  max2[2]=sigma;
  max2[3]=max_dummy;
  max2[4]=MAXIDE;
  error[1]=ModifyNodeZScoreS(n,dnode,dedge,edges,dedges,indirect,linker_edge,linker_degree,edgescr,d_edgescr,n_edgescr,dnodescr,swap_edges,rnd,max2);
