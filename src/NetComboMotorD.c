...
float  *NetComboMotorD(edges,size2,interaction,protein,nodescr,average_nodescr,rms_nodescr,n_nodescr,edgescr,average_edgescr,rms_edgescr,n_edgescr,maxmin,distribution,ndstr,linker,rnd,tolerance,threshold,thero,iteration,diter)
...
float    *nodescr,*edgescr,*average_nodescr,*average_edgescr,*rms_nodescr,*rms_edgescr;
{
 ...
 float     *NetScoreMotor(),*NetShortMotor(),*NetZScoreMotorD();
 float     *fvector();
 int       *ivector();
 ...
 iteration_netshort=iteration[3];
  ...
  printf("\n*******************************\n");
  printf("RUNING NetCombo Dynamic \n");
  printf("*******************************\n");
...
// Print Partial Modification by NetScore
 ...
 } // End calculation by NetScore

// ******************************************************************************** 
// Calculate NetZScore (dynamic) 

 if (iteration_netzscore>0){

 launch = clock();
 error_dummy= NetZScoreMotorD(edges,size2,interaction,protein,nodescr_dummy,average_nodescr_dummy,rms_nodescr_dummy,n_nodescr_dummy,edgescr_dummy,average_edgescr_dummy,rms_edgescr_dummy,n_edgescr_dummy,maxmin,distribution,ndstr,linker,rnd,tolerance,threshold,thero,iteration_netzscore,diter);
 for(i=0;i<MAXERROR;i++)if (error_dummy[i]>0 && error[i]<error_dummy[i]>0)error[i]=error_dummy[i];
