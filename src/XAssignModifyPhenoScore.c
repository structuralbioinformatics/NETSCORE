...
void  XAssignModifyPhenoScore(protein,interaction,xprobe,xsimilar,xPscore,nodescr,size,edges,number_phe,number_samples,xmethod,nref)
node      *protein;
edge      *interaction;
express   *xprobe;
float    **xPscore,**xsimilar,*nodescr;
int        size,edges,number_phe,number_samples[],nref,*xmethod;
{

 int    i,j,k,kk,ii,jj,sum,n1,n2,m1,m2;
 float *data1,*data2,prob,pb,statistic,maxndscr;
 float *fvector();
 ...
 maxndscr=1.0;
 for (i=0;i<size;i++){
	 if (nodescr[i]<0)nodescr[i]=0;
	 if (maxndscr<nodescr[i])maxndscr=nodescr[i];
 }
 for (i=0;i<size;i++){nodescr[i]=nodescr[i]/maxndscr;}
 switch (xmethod[17]){
  case 0:
   for (i=0;i<size;i++){
     printf("%10d\t%10s [%10.5f] => [%10.5f]\n",i,protein[i].name1,protein[i].copy.score,(xPscore[i][nref]*(protein[i].copy.score+nodescr[i])));
     protein[i].copy.score=xPscore[i][nref]*(protein[i].copy.score+nodescr[i]);
     }
  break;
  case 1:
   for (i=0;i<size;i++){
  ...
  case 2:
   for (i=0;i<size;i++){
     printf("%10d\t%10s [%10.5f] => [%10.5f]\n",i,protein[i].name1,protein[i].copy.score,((1.0+xPscore[i][nref])*(protein[i].copy.score+nodescr[i])));
     protein[i].copy.score=(1.0+xPscore[i][nref])*(protein[i].copy.score+nodescr[i]);
     }
  break;
  default:
   for (i=0;i<size;i++){
