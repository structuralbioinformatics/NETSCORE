...
float  *NetScoreMotor(edges,size2,interaction,protein,nodescr,average_nodescr,rms_nodescr,n_nodescr,edgescr,average_edgescr,rms_edgescr,n_edgescr,maxmin,distribution,ndstr,linker,rnd,tolerance,threshold,thero,iteration,diter)
...
float    *nodescr,*edgescr,*average_nodescr,*average_edgescr,*rms_nodescr,*rms_edgescr;
{
 ...
 void      free_matrix();
  ...
  while ( rho >= tolerance  && (n < iteration || iteration==0)){
   ...
   if (iteration>0) error=ModifyScore(n,diter,interaction,protein,dnode,dedge,edges,size,dedges,dsize,linker,edgescr,nodescr,dnodescr,d_edgescr,n_edgescr,d_nodescr,n_nodescr,maxmin);
  ...
  for(k=0;k<size;k++){
    ...
    if ((rms_nodescr[k]/nstep-average_nodescr[k]*average_nodescr[k])>1.0e-10){rms_nodescr[k]=sqrt(rms_nodescr[k]/nstep-average_nodescr[k]*average_nodescr[k]);}else{rms_nodescr[k]=0.0;}
  ...
  for(k=0;k<edges;k++){
     ...
     if ((rms_edgescr[k]/nstep-average_edgescr[k]*average_edgescr[k])>1.0e-10){rms_edgescr[k]=sqrt(rms_edgescr[k]/nstep-average_edgescr[k]*average_edgescr[k]);}else{rms_edgescr[k]=0.0;}
