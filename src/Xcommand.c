...
#include "data.h"      /* Includes data handling and file reading/writing */
                       ...
                       /* from data.c. */
#include "cluster.h"   /* Contains the version number of the C Clustering */
                       /* Library. */
#include "command.h"   /* Contains the declaration of commandmain */
...
{ printf ("\n"
#ifdef HAVE_GUI
"Cluster 3.0, command line and GUI version,\n"
#else
...
{ char* jobname;
  ...
  if (!jobname)
  { printf("ERROR: Failed to allocate memory for job name\n");
    return NULL;
  }
...
static void
JustSave(char* jobname)
{ FILE* outputfile;
  char* filename;
  int n = 1 + strlen(jobname) + strlen(".ext");
  filename = malloc(n*sizeof(char));
  if (!filename)
  { printf("ERROR: Failed to allocate memory for file name\n");
    return;
  }
  sprintf(filename, "%s.nrm", jobname);
  outputfile = fopen(filename, "wt");
  if (!outputfile) printf("ERROR: Failed to open output file %s\n", filename);
  else
  { Save(outputfile, 0, 0);
    fclose(outputfile);
  }
...
{ FILE* outputfile;
  ...
  { printf("ERROR: More clusters than microarrays available\n");
    return;
  }
  if (genemetric)
  { int dummy = k;
...
{ int i = 1;
  ...
  { if (l) LogTransform();
    ...
    { if(k>0)
        ...
        Hierarchical(genemetric, arraymetric, method, jobname);
      else
        JustSave(jobname);
      free(jobname);
    }
