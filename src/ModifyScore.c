#include "ppin.h"
float *ModifyScore(iter,diter,interaction,protein,dnode,dedge,edges,size,dedges,dsize,linker,edgescr,nodescr,dnodescr,d_edgescr,n_edgescr,d_nodescr,n_nodescr,maxmin)
...
edge     *interaction,*dedge;
{
 float *error;
 float *fvector();
 float  ModifyNodeScore(),ModifyDual();
 float  frac,nfrac,max,min, maxe, mine, mind,minde,max_dummy, sigma, max2[5];
 int    i,j,n;
 int    linker_node,linker_edge,indirect,linker_degree;
 ...
 max_dummy=max+1.0;  // This is to avoid duplicating the transfer of NOde-Score to dual-edge
                     ...
                     //
 printf("\nScore-Flow on Nodes (protein) NetScore Iteration= %d\n",iter);
 ...
 max2[3]=minde;
 max2[4]=MAXID;
 error[0]=ModifyNodeScore(iter,protein,interaction,size,edges,indirect,linker_node,linker_degree,nodescr,d_nodescr,n_nodescr,edgescr,max2);
 ...
 // n=iter;
  ...
  max2[3]=max_dummy;
  max2[4]=MAXIDE;
  error[1]=ModifyNodeScore(n,dnode,dedge,edges,dedges,indirect,linker_edge,linker_degree,edgescr,d_edgescr,n_edgescr,dnodescr,max2);
